...
class users extends Controller
{
    ...
    {
        $records= self::$model::all()->where('phone','!=',null);
    ...
    public static function indexPageing(Request $request)
    {
      ...
      $records= self::$model::all()->where('phone','!=',null)->$sort($request->sortBy??"id",);    if($request->search){
        ...
        if($request->filter){
            if($request->filter == 'active' ){
                ...
                $records= $records->where('is_active',0);
            }
        }
        $totalPages= ceil($records->count()/config('helperDashboard.itemPerPage'));
